...
pub fn find_stracciatella_home() -> Result<PathBuf, String> {
    // dirs::home_dir() is unsupported on emscripten; $HOME is set by the web
    // shell (play/index.html) to the IDBFS-persisted /userdata mount.
    #[cfg(target_os = "emscripten")]
    let base = std::env::var("HOME")
        .map(PathBuf::from)
        .ok()
        .or_else(|| Some(PathBuf::from("/userdata")));
