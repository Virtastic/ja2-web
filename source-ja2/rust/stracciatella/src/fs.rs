...
pub fn resolve_existing_components(path: &Path, base: Option<&Path>, caseless: bool) -> PathBuf {
    ...
    // NB: push(component.as_os_str()) instead of push(&component): the
    // AsRef<Path>-for-&Component path miscompiles on wasm32-unknown-emscripten
    // (rustc nightly-2024-09-01), producing NUL bytes in the path.
    joined_path
        .components()
