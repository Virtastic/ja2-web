...
ScreenID LaptopScreenHandle()
{
	...
	{ //Everything is set up to start the transition animation.
		...
		// Web: skip the 1s zoom busy-wait (RefreshScreen inside a GetClock loop
		// freezes the browser and can't repaint mid-loop). Fires on every laptop open.
		iRealPercentage = 100;
#endif
...
#ifdef __EMSCRIPTEN__
			// Web: skip the 1s zoom busy-wait (freezes the browser; fires on every laptop close).
			iRealPercentage = 0;
#endif
