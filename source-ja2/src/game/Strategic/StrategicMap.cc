...
void BeginLoadScreen( )
{
	...
	if( guiCurrentScreen == MAP_SCREEN && !(gTacticalStatus.uiFlags & LOADING_SAVED_GAME) && !AreInMeanwhile() )
	{
		...
		// Skip the ~2s zoom-transition busy-wait on web: it spins on real time
		// calling RefreshScreen() without returning to the browser, which freezes
		// the tab (and the browser can't even repaint the animation mid-loop).
		// Starting at 100% makes the loop below run zero iterations.
		iPercentage = 100;
#endif
