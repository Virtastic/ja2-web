...
static bool s_doGameCycles{true};

// One SDL event, shared between the desktop loop and the emscripten pump.
...
// Browser main-loop pump: the page drives this via a MessageChannel+RAF
// scheduler (see play/pump.js). Each call drains pending SDL events and runs
// one game cycle if the 144Hz deadline has passed; sleeping is impossible on
// the browser main thread, so pacing is early-return instead of sleep_until.
extern void UpdateJA2Clock();

extern "C" EMSCRIPTEN_KEEPALIVE void ja2_pump_frame()
{
	UpdateJA2Clock();
	...
	if (s_doGameCycles)
	{
		...
		static std::chrono::steady_clock::time_point s_nextCycle{};
		auto const now = std::chrono::steady_clock::now();
		if (now >= s_nextCycle)
		{
			FPS::GameLoopPtr();
			s_nextCycle = now + targetResolution;
		}
	}

	// Tell the page which screen is active, so it can present the tactical
	// (GAME_SCREEN) at full browser resolution while zooming the fixed 640x480
	// UI screens to fill (see play/index.html layoutCanvas). Fires only on change.
	static ScreenID s_lastScreen = ERROR_SCREEN;
	if (guiCurrentScreen != s_lastScreen)
	{
		s_lastScreen = guiCurrentScreen;
		EM_ASM({ if (window.__ja2SetScreen) window.__ja2SetScreen($0); }, (int)guiCurrentScreen);
	}
}

static void MainLoop()
{
	// Init is complete: tell the page it may start pumping frames, then keep
	// the runtime (and the heap-resident game state) alive after main()
	// unwinds; the JS pump takes over from here.
