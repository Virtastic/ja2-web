...
#ifdef __EMSCRIPTEN__
// The emscripten SDL2 audio callback runs on the main browser thread (via a
// ScriptProcessorNode), so a long game frame can starve it. A deeper buffer
// hides that jank at the cost of a little latency.
#define SOUND_SAMPLES   4096
#else
#define SOUND_SAMPLES   1024
#endif

// Threshold from which a sound is streamed from file instead of from memory.
#ifdef __EMSCRIPTEN__
// Never stream on wasm: streamed decode reads the Rust VFS from the buffer-
// service worker thread. Force everything (incl. music) fully into memory so
// the VFS is only ever touched on the main/load thread. RAM is plentiful (4GB cap).
#define SOUND_FILE_STREAMING_THRESHOLD (256 * 1024 * 1024)
#else
